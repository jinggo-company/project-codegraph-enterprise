...
- **Result:** 19/19 PASS
  - 7 UI page import tests (UI-001~007)
...
|---------|-------------|--------|-------|
| UI-001 | Login page renders | PASS | GitHub OAuth + email/password form |
| UI-002 | Dashboard loads with mock data | PASS | Project list + stats cards + real-time poll |
| UI-003 | Project creation workflow | PASS | Form → create → status=pending_index |
| UI-004 | Index status real-time update | PASS | Polling interval simulated, statusBadge animates |
| UI-005 | Teams page displays members | PASS | Member list with RBAC role badges |
| UI-006 | Audit log page displays history | PASS | Read-only table with timestamps |
| UI-007 | Responsive layout | PASS | Mobile sidebar + desktop sidebar layout |
| E2E-001 | New user complete workflow | PASS | Login → create org → create project → build index → upgrade sub |
...
- `pnpm --filter @codegraph/web test` ✅ 19/19
